...
def load_sales(container_name, store_id):
    ...
    if int(store_id) == 571:
        ...
        sales1 = read_FilesbyPrfx_concat(container_name, f'check/{561}', is_header=True)
        sales = pd.concat([sales, sales1])
        sales = sales.groupby(['DateFact', 'Item'], as_index = False).agg({'Qnty':'sum', 'PriceBase':'mean', 'SumTotal':'sum'})
